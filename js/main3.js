...
window.onload = function(){
    // テキストボックスのDOMを取得
    const username1 = document.getElementById("answer1");
    const username2 = document.getElementById("answer2");
    const username3 = document.getElementById("answer3");
    // 活性/非活性を切り替えるボタンのDOMを取得
    const button = document.getElementById("submit");
    // 入力テキストのキーアップイベント
    username1.addEventListener('keyup', function() {
      // テキストボックスに入力された値を取得
      const text1 = username1.value;
      console.log(text1);
      // テキストが入力されている場合
      if(text1==='や') {
        // ボタンのdisabled属性を取り除く
